...
const TableCart = ({ products = [], onClearCart }) => {
  ...
  return (
    <>
      {cart.length > 0 && (
        <div
          className={`flex justify-end py-1 pr-2  ${
            isDarkMode
              ? "bg-slate-800"
              : "bg-gray-100"
          }`}
        >
          <button
            onClick={onClearCart}
            className={`flex items-center gap-1 text-xs font-medium rounded-md px- py-1 transition-colors duration-200 ${
              isDarkMode
      ...
      >
        ...
        <div className="max-h-64 overflow-y-auto scroll-container">
          <table className="text-left table-auto w-full border-collapse">
            <thead>
              <tr
                className={`text-sm md:text-base sticky top-0 bg-opacity-90 backdrop-blur-md ${
                  isDarkMode
              ...
              {cart.length > 0 ? (
                cart.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <td className="py-2 px-4 truncate text-sm md:text-base">
                        {product.title
                      ...
                      <td className="px-2 text-sm md:text-base">{item.qty}</td>
                      <td className="px-2 text-sm md:text-base">
                        ${(product.price * item.qty).toFixed(2)}
                      </td>
                      <td className="px-2 text-center text-sm md:text-base">
                        <button
